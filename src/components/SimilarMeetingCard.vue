...
<template>
  <article
    ...
    class="rounded-2xl border border-slate-600 bg-slate-900/60 p-5"
  >
    <h3 class="mb-3 text-base font-semibold text-slate-200">유사 회의 발견</h3>
    <ul class="space-y-3">
      <li
        v-for="m in meetings"
        :key="m.id"
      ...
      >
        ...
        <p v-if="m.decision" class="mt-2 text-sm text-slate-400">
          결정: {{ m.decision }}
        </p>
